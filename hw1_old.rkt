...
(define (parse-defn s-exp)
  (match s-exp
    [`{deffun ,args ,body}  ; `{f x} `{twice x}
     (fundef (first args) (rest args) (parse body))  ; f  x  (parse `{twice x})
     ]
    ))
...
(define (interp an-f1wae fundefs)
  (type-case F1WAE an-f1wae
    ...
    [app (fun-name values)
         ...
         (interp
          (for/fold ([init-body body])  ; '{}
                    ([value (map
                             (lambda (x)
                               (interp x fundefs))
                             values )]  ; actual parameter
                     [param (fundef-param-names the-fundef)]  ; formal parameters
                     )
